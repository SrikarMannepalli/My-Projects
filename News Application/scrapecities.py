...
page=requests.get('http://indianexpress.com/section/cities/')
...
for i in range(0,total_num-1):
	...
	for j in range(0,length-1):
		...
		mypara=str(mypara+"\n"+pp)
	cur.execute('INSERT INTO datacities(heading,link,imagelink,imagewidth,imageheight,article) VALUES(%s,%s,%s,%s,%s,%s)',(str(headings[i]),str(links[i]),str(imagelinks[i]),str(imagewidths[i]),str(imageheights[i]),mypara))
	con.commit()

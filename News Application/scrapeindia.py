...
page=requests.get('http://indianexpress.com/section/india/')
...
cur=con.cursor()
cur.execute('DROP TABLE IF EXISTS dataindia')
cur.execute('CREATE TABLE dataindia(id INT(11) AUTO_INCREMENT PRIMARY KEY,heading VARCHAR(1000),link VARCHAR(1000),imagelink VARCHAR(1000),imagewidth INT(11),imageheight INT(11),article VARCHAR(13000));')
for i in range(0,total_num):
	...
	for j in range(0,length-1):
		...
		mypara=mypara+"\n"+pp
		mypara=str(mypara)
	cur.execute('INSERT INTO dataindia(heading,link,imagelink,imagewidth,imageheight,article) VALUES(%s,%s,%s,%s,%s,%s)',(str(headings[i]),str(links[i]),str(imagelinks[i]),str(imagewidths[i]),str(imageheights[i]),mypara))
	con.commit()
